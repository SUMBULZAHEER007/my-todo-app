#!/usr/bin/env python3
"""
Simple In-Memory Console Todo App
...
class TodoApp:
    ...
    def update_task(self, task_id: int, new_title: Optional[str] = None, new_description: Optional[str] = None) -> str:
    ...
    while True:
        try:
            ...
            if command == "quit":
                ...
                if len(parts) < 2:
                    print("Error: Please provide a task title. Usage: add <title> [description]")
                else:
                    # Split the remaining part to get title and description
                    add_parts = parts[1].split(maxsplit=1)
                    title = add_parts[0]
                    description = add_parts[1] if len(add_parts) > 1 else ""
                    result = app.add_task(title, description)
                    print(result)
            elif command == "view":
                print(app.view_tasks())
            elif command == "update":
                if len(parts) < 2:
                    print("Error: Please provide task ID, title, and optionally description. Usage: update <id> <title> [description]")
                else:
                    # Split the remaining part to get ID, title, and description
                    update_parts = parts[1].split(maxsplit=2)
                    if len(update_parts) < 2:
                        print("Error: Please provide both task ID and new title. Usage: update <id> <title> [description]")
                    else:
                        try:
                            task_id = int(update_parts[0])
                            new_title = update_parts[1]
                            new_description = update_parts[2] if len(update_parts) > 2 else None
                            result = app.update_task(task_id, new_title, new_description)
                            print(result)
                        except ValueError:
